...
const fetcher = (...args) => fetch(args[0], { credentials: 'include' }).then(res => {
    if (!res.ok) {
    ...
    return (
        ...
        >
            <div className="flex items-center gap-1">
                ...
                {isActive ? (
                    isAsc ? <ArrowUp size={14} className="text-sky-500" /> : <ArrowDown size={14} className="text-sky-500" />
                ) : (
...
export default function MainAdminCandidatures() {
    ...
    const { data: rawData, error, isLoading } = useSWR(`${ApiUrl}/candidature/`, fetcher);
    ...
    const filteredCandidatures = useMemo(() => {
        return candidatures.filter(c => {
            ...
            if (filterProfil !== 'Tous les profils' && c.profilPostule !== filterProfil) {
                return false;
            }
        ...
        <div className="h-full w-full bg-white rounded-xl shadow-lg p-4 sm:p-6 lg:p-8 
            ...
            <div className="flex flex-col sm:flex-row flex-wrap lg:flex-nowrap items-start sm:items-center gap-3 lg:gap-4 py-3 border-b border-gray-100 pb-4">
                ...
                <div className="flex items-center gap-2 w-full sm:w-auto">
                    <span className="text-sm font-semibold text-gray-700 hidden sm:inline lg:block lg:w-auto">Profil :</span>
                    <select
                        value={filterProfil}
                ...
                <div className="flex items-center gap-2 w-full sm:w-auto">
                    <span className="text-sm font-semibold text-gray-700 hidden sm:inline lg:block lg:w-auto">Niveau :</span>
                    <select
                        value={filterNiveau}
                ...
                <div className="flex items-center gap-2 w-full sm:w-auto">
                    <span className="text-sm font-semibold text-gray-700 hidden sm:inline lg:block lg:w-auto">Date :</span>
            ...
            <div className="flex-1 overflow-x-auto mt-4">
                {sortedCandidatures.length > 0 ? (
                            ...
                            <table className="table table-zebra w-full min-w-[800px] ">
                                <thead className='text-black text-sm'>
                                    <tr className="bg-gray-50">
                                        ...
                                        <SortableHeader title="Statut" column="statut" sortColumn={sortColumn} sortOrder={sortOrder} onSort={handleSort} />
                                    </tr>
                                </thead>
                                <tbody>
                    ...
                    <div className="text-center text-gray-500 font-medium p-10">
                        <p>Aucune candidature ne correspond aux critères de filtre sélectionnés.</p>
                    </div>
                )}
            </div>
        </div>
