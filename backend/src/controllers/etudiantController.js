...
class EtudiantController {
  ...
  static async createEtudiant(req, res) {
    try {
      ...
      await NotificationController.addNotification({
        UtilisateurId: null,
        message: `On vous souhaite la bienvenue ! Cette plateforme vous sera très utile. Mettez vous à l'aise, prenez vos marques et consultez déjà tous nos offres disponibles !`,
        type: "actualité",
  ...
  static async deleteEtudiant(req, res) {
    ...
    try {
      const deletedEtudiant = await Etudiant.sequelize.transaction(
        async (t) => {
      ...
      if (error.message === "not_found") {
        return res
          .status(404)
          .json({
            message: `L'étudiant avec l'identifiant ${id} n'existe pas.`,
          });
      }
  ...
  static async updateEtudiant(req, res) {
    ...
    try {
      const etudiant = await Etudiant.sequelize.transaction(async (t) => {
        const [affectedRows] = await Etudiant.update(req.body, {
          where: { id },
          transaction: t,
        });
  ...
  static async getEtudiantByUserId(req, res) {
    ...
    try {
      ...
      if (!etudiant) {
        return res
          .status(404)
          .json({ message: "Aucun étudiant trouvé pour cet utilisateur." });
      }
