...
class Auth {
    ...
    static async callback(req, res) {
        ...
        if (isNew && googleProfile) {
            ...
            return res.redirect("http://localhost:5173/auth/sign-up/set-password");
        }
    ...
    static async registerGoogle(req, res) {
        try {
            if (!req.cookies.google_temp) return res.status(400).json({ error: "Données temporaires manquantes." });
            ...
            console.error(err);
            return res.status(500).json({ message: "Inscription échouée.", data: err });
        }
    }
    ...
    static async register(req, res) {
        ...
        try {
            ...
            res.cookie('signup_temp', JSON.stringify({ email, passwordHash, verificationCode: code, verificationExpires: expires }), {
                httpOnly: true, maxAge: 10*60*1000, secure: process.env.PROD_COOKIE_SECURE === 'true', sameSite: 'lax'
            });
    ...
    static async verifyCode(req, res) {
        const { code } = req.body;
        try {
            if (!req.cookies.signup_temp) return res.status(400).json({ message: 'Inscription expirée.' });
    ...
    static async resendCode(req, res) {
        try {
            ...
            res.cookie('signup_temp', JSON.stringify(tempData), { httpOnly: true, maxAge: 10*60*1000, secure: process.env.PROD_COOKIE_SECURE === 'true', sameSite: 'lax' });
